...
def get_args():
    ...
    parser.add_argument(
        "--epochs", type=int, default=101, help="Number of epochs to train with"
    )
    parser.add_argument(
...
def main(n):
    args = get_args()
    name = 'CH/' + args.dataset + '/ResNet_' + str(n)
    ...
    rtpt = RTPT(name_initials='HS',
                experiment_name='CH/' + args.dataset + '/ResNet_' + str(n), max_iterations=args.epochs)
    rtpt.start()
