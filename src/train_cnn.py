...
def get_args():
    ...
    parser.add_argument(
        "--epochs", type=int, default=300, help="Number of epochs to train with"
    )
    parser.add_argument(
...
def main(n):
    ...
    if not args.small_data:
        ...
        base = 'small_KP/'
    name = base + args.dataset + '/ResNet_' + str(n)
    ...
    rtpt = RTPT(name_initials='HS',
                experiment_name=base + args.dataset + '/ResNet_' + str(n), max_iterations=args.epochs)
    rtpt.start()
