...
def get_args():
    ...
    parser.add_argument("--uc_top", type=int, default=3,
                        help="The accept number for unclassified clauses.")
    parser.add_argument("--uc_good_top", type=int, default=3,
                        help="The accept number for unclassified good clauses.")
    parser.add_argument("--sc_good_top", type=int, default=3,
                        help="The accept number for sufficient good clauses.")
    parser.add_argument("--nc_good_top", type=int, default=3,
...
def train_and_eval(args, pm_prediction_dict, val_pos_loader, val_neg_loader, writer, rtpt, exp_output_path):
    ...
    while max_step < args.t_beam:
        ...
        if len(bs_clauses['sn']) > 0:
            log_utils.add_lines(f"found sufficient and necessary clause: {bs_clauses['sn']}.", args.log_file)
            clauses += logic_utils.extract_clauses_from_bs_clauses(bs_clauses['sn'])
            break
        elif len(bs_clauses['sn_good']) > 0:
            log_utils.add_lines(f"found quasi-sufficient and necessary clause: {bs_clauses['sn_good']}.", args.log_file)
