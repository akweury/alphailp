...
from percept import SlotAttentionPerceptionModule
...
def get_args():
    ...
    parser.add_argument(
        "--dataset",
        choices=["clevr-hans0", "clevr-hans1", "clevr-hans2"],
        help="Use MNIST dataset",
    )
    parser.add_argument("--dataset-type", default="clevr", help="kandinsky or clevr")
    parser.add_argument(
        "--perception-model",
...
def main(n):
    args = get_args()
    name = 'CH/' + args.dataset + '/SAMLP' + str(n)
    ...
    net = SlotAttentionPerceptionModule(e=10, d=19, device=device)
    net = net.to(device)
    predict_net = MLP(in_channels=10 * 19, out_channels=1)
    predict_net.to(device)
    ...
    rtpt = RTPT(name_initials='HS',
                experiment_name='CH/' + args.dataset + '/SAMLP_' + str(n), max_iterations=args.epochs)
    rtpt.start()
