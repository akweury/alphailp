...
def to_line_tensor(point_groups):
    ...
    line_tensor[tensor_index["slope"]] = torch.from_numpy(line_model.coef_.reshape(-1))
    line_tensor[tensor_index["probability"]] = 1 - torch.abs(
        torch.from_numpy(line_model.predict(point_groups[:, 0:1])) - point_groups[:, 2:]).sum() / point_groups.shape[0]

    line_tensor[tensor_index["is_line"]] = 1
    line_tensor[tensor_index["is_circle"]] = 0
...
def to_circle_tensor(point_groups, center, r):
    ...
    circle_tensor[tensor_index["slope"]] = 0
    circle_tensor[tensor_index["probability"]] = 1 - torch.abs(
        torch.sqrt(((point_groups[:, [0, 2]] - center) ** 2).sum(dim=1)) - r).sum() / point_groups.shape[0]
    circle_tensor[tensor_index["is_line"]] = 0
    circle_tensor[tensor_index["is_circle"]] = 1
